...
def create_region_table(paths, fits_file, reg_file, base_reg_fit_file, instr):
    ...
    out_path = paths['save_path'] / f'temp_table_{instr}_{mode}.fits'
...
def update_region_table(paths, reg_file, base_reg_fit_file, instr):
    ...
    """
    mode = base_reg_fit_file[-8:-5]
    hdul1 = fits.open(paths['save_path'] / f'temp_table_{instr}_{mode}.fits', mode='update')
    table1 = hdul1[1]
    ...
    for i, row in enumerate(row1):
        ...
        table1.data[i][key_x][0] = format(x, '.12f')
...
def rename_tables(paths):
    """
    Renames temporary region FITS files to their final format for each instrument.

    Parameters:
        paths (dict): Dictionary of path values.
    """
    for file in os.listdir(paths['save_path']):
        if not file.endswith('.fits'):
            continue
        tokens = file.split('_')
        mode = tokens[-1].replace('.fits', '')
        if 'm1' in file:
            newname = f'mos1S001-bkg_region-{mode}.fits'
        elif 'm2' in file:
            newname = f'mos2S002-bkg_region-{mode}.fits'
        elif 'pn' in file:
            newname = f'pnS003-bkg_region-{mode}.fits'
        else:
            continue
        os.rename(paths['save_path'] / file, paths['final_path'] / newname)
